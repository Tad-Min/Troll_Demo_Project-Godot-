...
func _spawn_shuriken(pos: Vector2, dir: Vector2) -> void:
	if shuriken_scene:
		...
		if s.has_method("setup"):
			s.setup(dir, shuriken_speed,shrink_duration, shuriken_size, min_scale)
		else:
			push_warning("⚠️ Script shrunkriken chưa có hàm setup(dir, speed, duration, size, min_shrink)")
